...
WITH params AS (
    SELECT NOW()                                              AS ahora,
        ...
        CAST(:transfer_donor_stock_min AS int)                       AS transfer_donor_stock_min
),
...
recep_90d AS (
    ...
    WHERE r.bsale_office_id = ANY(p.sucursales_objetivo)
      ...
      AND rd.quantity <= p.qty_sanity_limit  -- ★ P22: tope de sanidad (códigos de barras tipeados como cantidad)
    GROUP BY 1, 2
),
primera_recep_total AS (
    ...
    CROSS JOIN params p
    WHERE r.bsale_office_id = ANY(p.sucursales_objetivo)
      AND rd.quantity <= p.qty_sanity_limit  -- ★ P22: tope de sanidad
    GROUP BY 1, 2
),
...
consumos_lifetime AS (
    ...
    CROSS JOIN params p
    WHERE c.bsale_office_id = ANY(p.sucursales_objetivo)
      AND cd.quantity <= p.qty_sanity_limit  -- ★ P22: tope de sanidad (las "correcciones" de recepciones corruptas también eran absurdas)
    GROUP BY 1, 2
),
...
metricas AS (
    SELECT c.*, tc.cat_ventas, tc.cat_tdpv,
        ...
        CASE
            WHEN c.stock_disponible > 0 THEN (SELECT ventana_recent_dias FROM params)
...
SELECT
    ...
    CASE
        WHEN donor.stock_disponible >= (SELECT transfer_donor_stock_min FROM params)
             AND donor.proy_mes >= 10
             AND COALESCE(donor.proy_30d_reciente, 0) < (SELECT lote_frenado_proy30_max FROM params)
        ...
        WHEN donor.unds_vendidas = 0
             AND donor.stock_disponible >= (SELECT transfer_donor_stock_min FROM params)
             AND donor.primera_recepcion < (CURRENT_DATE - ((SELECT ventana_dead_dias FROM params) * INTERVAL '1 day'))
            THEN '💀 STOCK MUERTO (sin venta en ventana operativa)'
    ...
    CASE
        WHEN recip.stock_disponible = 0 AND recip.proy_mes >= (SELECT proy_mes_alta FROM params)
        ...
        WHEN recip.dias_cobertura IS NOT NULL
             ...
             AND recip.proy_mes >= (SELECT proy_mes_alta FROM params)
    ...
    GREATEST(1, LEAST(
        CASE
            WHEN COALESCE(donor.proy_30d_reciente, 0) < (SELECT lote_frenado_proy30_max FROM params) OR donor.unds_vendidas = 0
    ...
    CASE
        WHEN recip.stock_disponible = 0 AND recip.proy_mes >= (SELECT proy_mes_alta FROM params) THEN 1
...
WHERE
    ...
    donor.stock_disponible >= (SELECT transfer_donor_stock_min FROM params)
    AND (
        -- exceso
        ...
        (donor.proy_mes >= 10
         ...
         AND COALESCE((CURRENT_DATE - donor.primera_recepcion::date), 0) >= 90)
        OR
        -- muerto: sin ventas en ventana operativa con stock recibido hace tiempo
