...
Dos endpoints, ambos por-SKU, livianos:
  ...
  GET /matrix-sim/sku-history/{sku}?days=180&office_id=...
...
router = APIRouter(prefix="/matrix-sim", tags=["matrix-sim"])
...
async def get_sku_history(
    ...
    res = await db.execute(
        text(sql),
        {"sku": sku, "days": days, "office_id": office_id, "office_ids": list(OFFICES_TIENDA)},
    )
    points = [
        {
