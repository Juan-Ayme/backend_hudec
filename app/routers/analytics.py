...
from app.routers.config_admin import get_goals, goal_for_month, set_goals_month
...
async def top_products(
    ...
    if office_id is not None:
        office_filter = "doc.bsale_office_id = :office_id"
        params: dict[str, Any] = {"dfrom": dfrom, "dto": dto, "limit": limit, "office_id": office_id}
    else:
        office_filter = _OFFICE_FILTER_DOC
        params = {"dfrom": dfrom, "dto": dto, "limit": limit}
    query = f"""
...
async def _period_metrics(
    ...
    # Excluimos gratuidades del cálculo de "unds vendidas" porque son regalos,
    # pero las reportamos aparte (interesante para análisis de pérdida).
    q2 = f"""
        SELECT
...
async def ticket_anatomy(
    ...
    if office_id is not None:
        ...
        cur_params  = {"dfrom": cur_dfrom,  "dto": cur_dto,  "office_id": office_id}
        prev_params = {"dfrom": prev_dfrom, "dto": prev_dto, "office_id": office_id}
    else:
        of_plain = _OFFICE_FILTER_PLAIN
        of_doc   = _OFFICE_FILTER_DOC
        cur_params  = {"dfrom": cur_dfrom,  "dto": cur_dto}
        prev_params = {"dfrom": prev_dfrom, "dto": prev_dto}

    cur  = await _period_metrics(db, cur_dfrom,  cur_dto,  of_plain, of_doc, cur_params)
    prev = await _period_metrics(db, prev_dfrom, prev_dto, of_plain, of_doc, prev_params)
...
async def stockouts(
    ...
    if office_id is not None:
        ...
        params = {}
    params.update({"dwin": d_window, "dlb": d_lookback, "limit": limit})
    ...
    query = f"""
        WITH ventas AS (
            ...
            FROM document_details dd
            JOIN documents doc ON doc.bsale_document_id = dd.bsale_document_id
            WHERE (doc.emission_date AT TIME ZONE '{_TZ}')::DATE >= :dlb
              AND COALESCE(doc.is_credit_note, FALSE) = FALSE
              AND {_OFFICE_FILTER_DOC}
        ...
        base AS (
            ...
            FROM stock_levels sl
            JOIN variants v  ON v.bsale_variant_id = sl.bsale_variant_id AND v.is_active
            JOIN products p  ON p.bsale_product_id = v.bsale_product_id
                            AND p.is_active AND p.stock_control
            LEFT JOIN v_products_full vpf ON vpf.bsale_product_id = v.bsale_product_id
            LEFT JOIN offices o          ON o.bsale_office_id   = sl.bsale_office_id
...
@router.get("/goals")
async def read_goals(db: AsyncSession = Depends(get_db)) -> dict:
    """Todas las metas de venta configuradas (manual), keyed por mes 'YYYY-MM'."""
    return {"goals": await get_goals(db)}


@router.put("/goals")
async def write_goals(body: GoalsBody, db: AsyncSession = Depends(get_db)) -> dict:
    ...
    for k, v in body.offices.items():
        month_goals[str(k)] = v
    goals = await set_goals_month(db, body.month, month_goals)
...
async def sales_vs_goal(
    ...
    if office_id is not None:
        office_filter = "bsale_office_id = :office_id"
        params: dict[str, Any] = {"dfrom": dfrom, "dto": dto, "office_id": office_id}
        scope = [office_id]
    else:
        office_filter = _OFFICE_FILTER_PLAIN
        params = {"dfrom": dfrom, "dto": dto}
    ...
    off_rows = (await db.execute(
        text("SELECT bsale_office_id, name FROM offices WHERE bsale_office_id = ANY(:ids)"),
        {"ids": scope},
    )).mappings().all()
...
async def weekly_board(
    ...
    """
    kpi = await kpis(days=days, office_id=office_id, db=db)
    serie = await sales_by_day(days=days, office_id=office_id, db=db)
    categorias = await sales_by_category(days=days, office_id=office_id, db=db)
    quiebres = await stockouts(office_id=office_id, demand_window_days=30, limit=2000, db=db)
    meta = await sales_vs_goal(month=month, office_id=office_id, db=db)
...
async def compras_catalogo_excel(
    ...
    # 1) Matriz JERÁRQUICA (04b) — misma fuente que el Excel de /ventas-jerarquicas.
    #    Filtramos por: a) sucursal si pidieron, b) solo SKUs en quiebre real
    #    (críticos + altas). De ahí construimos el workbook ejecutivo (resumen +
    #    hoja por departamento) usando el builder ya existente.
    result = await service.run_matrix(
        db, "04b",
        sucursal=None,   # filtramos por office_id abajo (Sucursal es texto, requiere mapear)
        limit=None,
    )
    cols = result["columns"]
    rows = result["rows"]

    # Resolver nombre de sucursal si se filtra por office_id (la matriz expone "Sucursal" como texto).
    suc_name: str | None = None
    if office_id is not None:
        srow = (await db.execute(
            text("SELECT name FROM offices WHERE bsale_office_id = :oid"),
            {"oid": office_id},
        )).first()
        suc_name = srow[0] if srow else None

    # Filtrar a quiebre real: misma lógica que _classify_severidad_accion_causal,
    # nos quedamos con severidades 🔴 Crítico y 🟠 Alta.
    ...
    cats = await sales_by_category(days=days, office_id=office_id, db=db)
...
async def daily_report_excel(
    ...
    if office_id is not None:
        office_filter = "bsale_office_id = :office_id"
        params: dict[str, Any] = {"dfrom": first, "dto": today + timedelta(days=1), "office_id": office_id}
        scope = [office_id]
    else:
        office_filter = _OFFICE_FILTER_PLAIN
        params = {"dfrom": first, "dto": today + timedelta(days=1)}
        scope = list(OFFICE_IDS)
    ...
    serie_office_rows = (await db.execute(
        text(serie_office_q),
        {"dfrom": first, "dto": today + timedelta(days=1), "scope_ids": list(scope)},
    )).mappings().all()
